...
uint256 hashGenesisBlock("0x9b8ff569bea8d0fc3ed38508dbd6153f0f0889530210be3f6af32ec030245906");
...
const string strMessageMagic = "Fatcoin Signed Message:\n";
...
{
    // Fatcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Fatcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Fatcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Fatcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe811cc251ed5d41a1e09b43b71999dc6e83bb88f5d974a82e9e9ceca5f622b68");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "CNN.com 05/Mar/2014 Mom drives van, 3 kids into waves";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1394030903;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 931299747;

        if (fTestNet)
        {
            block.nTime    = 1394031006;
            block.nNonce   = 929897075;
        }
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Fatcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FatcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FatcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FatcoinMiner(CWallet *pwallet)
{
    printf("FatcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("fatcoin-miner");
    ...
    try { loop {
        ...
        printf("Running FatcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FatcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FatcoinMiner, pwallet));
}
